...
function Petition({onAddPetition, nickname, handleAccountBtnClick}) {
    ...
    return (
        <section className="petition-form">
            <div className="petition-form__title-area">
                <h1 id="petition-form" className="petition-form__title">Создать инициативу</h1>
                <p className="petition-form__subtitle">
                    {!currentUser.uid && `Чтобы подать инициативу, войдите в `}
                    <span  className="petition-form__subtitle-link" onClick={handleAccountBtnClick}>
                        {!currentUser.uid && `аккаунт`}
                    </span></p>
            </div>
